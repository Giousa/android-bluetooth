...
public class BTClient extends Activity {
	...
	private final static int REQUEST_ENABLE_BT = 2;    //打开蓝牙设备
	
	private final static String MY_UUID = "00001101-0000-1000-8000-00805F9B34FB";   //SPP服务UUID号
	
	private InputStream iStream;    //输入流，用来接收蓝牙数据
	private InputStream iStream1;    //输入流，用来接收蓝牙数据
    ...
    BluetoothSocket _socket1 = null;      //蓝牙通信socket 
    boolean _discoveryFinished = false;    
    boolean bRuning = false;
    boolean bThread = false;
    boolean bThread1 =false;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
            ...
            if (resultCode == Activity.RESULT_OK) {   //连接成功，由DeviceListActivity设置返回
                ...
                try{
                	if(_socket==null)
                		...
                		_socket1= _device.createRfcommSocketToServiceRecord(UUID.fromString(MY_UUID));
                ...
                try{
                	if(isBool)
                		iStream = _socket.getInputStream();   //得到蓝牙数据输入流
                	else
                		iStream1 = _socket1.getInputStream();   //得到蓝牙数据输入流
                	isBool = false;
            		}catch(IOException e){
            		...
            		if(bThread==false){
            			new ReadThread(iStream,bRuning).start();
            			bThread=true;
            		}else{
            			bRuning = true;
            		}
            		if(bThread1==true){
            			new ReadThread(iStream1,bRuning).start();
            			bThread1=true;
            		}else{
            			bRuning = true;
            		}
    	...
    	case REQUEST_ENABLE_BT:
    		...
    		if(resultCode == Activity.RESULT_OK){
    			...
    			Toast.makeText(this,R.string.bt_disable, Toast.LENGTH_SHORT).show();
    		}
    		break;
    	default:break;
    	}
    ...
    class ReadThread extends Thread{
        ...
        ReadThread(InputStream iStream, boolean bRun) {
  ...
  /* @Override
    ...
    public void onConnectButtonClicked(View v){ 
    	if(_bluetooth.isEnabled()==false){  //如果蓝牙服务不可用则提示
    		...
    		enableIntent.putExtra(BluetoothAdapter.EXTRA_DISCOVERABLE_DURATION, 300);
    		startActivityForResult(enableIntent,REQUEST_ENABLE_BT);
			//BTClient.this.startActivity(enableIntent);
